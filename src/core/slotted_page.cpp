...
types::Tuple core::SlottedPageManager::readTuple(core::Buffer_Pool_Manager& buffer_pool_manager,const uint32_t page_id, const uint32_t offset){
    const types::Frame* frame = buffer_pool_manager.fetch_page(page_id);
    ...
    if (key_overflow == true){
        uint32_t curr_overpage_id;
        std::memcpy(&curr_overpage_id, page.buffer + pos, sizeof(curr_overpage_id));
        pos += sizeof(curr_overpage_id);

        const types::OverflowPageHeader* curr_overflow_page_header;
        do{
            const types::Frame* frame = buffer_pool_manager.fetch_page(curr_overpage_id);

            //TODO: add header check
            types::OverflowPage overflow_page{frame->buffer};
            curr_overflow_page_header = overflow_page.header();

            const char* data_ptr = overflow_page.buffer + sizeof(types::OverflowPageHeader);
            std::string curr_val(data_ptr, curr_overflow_page_header->data_bytes);
            key.append(curr_val);

            curr_overpage_id = curr_overflow_page_header->next_page_id;
        }while(!curr_overflow_page_header->last_page);
    }else{
...
uint32_t core::SlottedPageManager::writeTuple(core::Buffer_Pool_Manager& buffer_pool_manager, const uint32_t page_id, const types::Tuple& tuple){
    types::Frame* frame = buffer_pool_manager.fetch_page_mut(page_id);
    types::SlottedPage page{frame->buffer};
