...
types::Tuple core::SlottedPageManager::readTuple(core::Buffer_Pool_Manager& buffer_pool_manager, const uint32_t page_id, const uint16_t offset){
    ...
    uint16_t pos = offset;
    const char* logicalSlotEnd = buffer + header.record_start_offset;
...
uint16_t core::SlottedPageManager::writeTuple(
        types::Frame* frame,
    ...
    if (tuple.val.size() * sizeof(char) > config::OVERFLOW_LIMIT){
        value_size = sizeof(value_overflow_page_id);
        value_overfow = 1;
    }else{
    ...
    if (full_data_size > total_free_space){
        throw std::runtime_error("Could not fit tuple into given page");
    }

    if (full_data_size > continuous_free_space){
        //TODO: implement compaction;
    }
    
    // start writing record backwards
    uint16_t pos = header.record_start_offset;
    // logical backend also includes the current slot entry which would written after the slot_end_offset hence, the extra uint16_t 
    char* logicalBackEnd = buffer + header.slot_end_offset + sizeof(uint16_t);

    const char* value_data = value_overfow
        ? reinterpret_cast<const char*>(&value_overflow_page_id)
        : value.data();
    ...
    const char* key_data = key_overfow
        ? reinterpret_cast<const char*>(&key_overfow_page_id)
        : key.data();
...
void core::SlottedPageManager::deleteTuple(core::Buffer_Pool_Manager& buffer_pool_manager, const uint32_t page_id, const uint32_t offset){
    return;
};

void core::SlottedPageManager::serializeSlottedPageHeader(char* buffer, const types::SlottedPageHeader& header){
    uint16_t pos = 0;
